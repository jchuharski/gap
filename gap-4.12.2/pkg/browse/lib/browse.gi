...
BrowseData.PositionInBrowseTable:= function( t, data )
    local wins, win, ws, scr, y, x, diff, leftmargin, result, i;
...
#T Is it currently possible to arrive here if not?
    ...
    if   y <= ws.topmargin then
      ...
      return [ "below" ];
    elif y <= ws.topmargin + ws.headerLength then
      # Note that the header can be wider than the table.
      diff:= ws.heightWidthWindow[2] - ws.headerWidth;
      if ws.leftmargin + ws.rightmargin <= diff then
        leftmargin:= ws.leftmargin;
      else
        leftmargin:= Maximum( Int( diff / 2 ), 0 );
      fi;
      if x <= leftmargin then
        return [ "left" ];
      elif x <= leftmargin + ws.headerWidth then
        return [ "header", [ y - ws.topmargin, x - leftmargin ] ];
      else
        return [ "right" ];
      fi;
    elif scr[1] - ws.bottommargin - ws.footerLength < y then
      # Note that the footer can be wider than the table.
      diff:= ws.heightWidthWindow[2] - ws.footerWidth;
      if ws.leftmargin + ws.rightmargin <= diff then
        leftmargin:= ws.leftmargin;
      else
        leftmargin:= Maximum( Int( diff / 2 ), 0 );
      ...
      if x <= leftmargin then
        return [ "left" ];
      elif x <= leftmargin + ws.footerWidth then
        return [ "footer", [ y - scr[1] + ws.bottommargin + ws.footerLength,
                             x - leftmargin ] ];
      else
        return [ "right" ];
      fi;
    elif x <= ws.leftmargin then
      return [ "left" ];
...
BrowseData.ShowTables := function( t )
    local mode, header, footer, ws, scr, rng, diff, leftmargin, brc;
    ...
    if not BrowseData.IsQuietSession( t.dynamic.replay ) then
      ...
      # Enter the header.
      # (It may be wider than the main part of the table.
      # In this case, put the header vertically centered into the window.)
      diff:= ws.heightWidthWindow[2] - ws.headerWidth;
      if ws.leftmargin + ws.rightmargin <= diff then
        leftmargin:= ws.leftmargin;
      else
      ...
      NCurses.PutLine( t.dynamic.window, ws.topmargin, leftmargin, header );
    fi;
    ...
    if not BrowseData.IsQuietSession( t.dynamic.replay ) then
      # Enter the footer.
      # (It may be wider than the main part of the table.
      # In this case, put the footer vertically centered into the window.)
      diff:= ws.heightWidthWindow[2] - ws.footerWidth;
      if ws.leftmargin + ws.rightmargin <= diff then
        leftmargin:= ws.leftmargin;
      else
      ...
      NCurses.PutLine( t.dynamic.window,
          scr[1] - ws.bottommargin - Length( footer ), leftmargin, footer );
